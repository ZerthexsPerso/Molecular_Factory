...
public class MachineLCDouble: MachineLC
{
    ...
    {
        if (recipeCatalogue.doubleLC[1]) activeRecipe = recipeCatalogue.doubleLC[1];
    }

    public override void OutputMaker()
    {
        string combinaison = "";
        if (inputStock1.amountInStock > 0 && inputStock2.amountInStock > 0 && inputStock3.amountInStock > 0)
        {
            combinaison += inputStock1.name + inputStock2.name + inputStock3;
        }
