...
public class MachineLCQuadruple : MachineLC
{
    ...
    {
        if (recipeCatalogue.quadrupleLC[1]) activeRecipe = recipeCatalogue.quadrupleLC[1];
    }

    public override void OutputMaker()
    {
        string combinaison = "";
        if (inputStock1.amountInStock > 0 && inputStock2.amountInStock > 0 && inputStock3.amountInStock > 0 && inputStock4.amountInStock > 0 && inputStock5.amountInStock > 0)
        {
            combinaison += inputStock1.name + inputStock2.name + inputStock3 + inputStock4 + inputStock5;
        }
    ...
    public override void PipeReceiver()
    {
        ...
        if (inputPipe5)
        {
            if (inputStock5.name == inputPipe5.itemInStock.name || inputStock5.name == "")
            {
                if (inputStock5.name == "") inputStock5.name = inputPipe5.itemInStock.name;
                inputStock5.amountInStock += pipeSpeed;
                inputPipe5.itemInStock.amountInStock -= pipeSpeed;
            }
        }
    }
